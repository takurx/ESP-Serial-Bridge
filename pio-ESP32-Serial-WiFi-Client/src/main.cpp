...
const int wdtTimeout = 30000;  // reboot after 30 seconds of no host
...
void setup() {
  ...
  // ESP32 (Arduino Core) 2.0系から3.0系への移行 | なんでも独り言
  // https://ehbtj.com/electronics/migrating-from-esp32-arduino-core-v2-to-v3/
  // 3.0系
  ...
  // 2.0系
  // timer = timerBegin(0, 1000000, true);                    //timer 1MHz
  timer = timerBegin(0, 16960, true);                    //timer 1MHz
  timerAttachInterrupt(timer, &resetModule, true);      //attach callback
  timerAlarmWrite(timer, wdtTimeout * 1000, true);     //set time in us
  timerAlarmEnable(timer);                        //enable interrupt
  debug.println("Watch dog timer Setup done");

  debug.println("while, setup");
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    debug.print(".");
  }
  timerWrite(timer, 0);  //reset timer (feed watchdog)
  debug.println("connected, setup");
  debug.print("IP address: ");
  debug.println(WiFi.localIP());
